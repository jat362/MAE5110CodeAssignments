"""InvertedPendulumWalker model.

State: state = [theta, angular_velocity]
  theta measured clockwise from upward vertical (matches visualize()'s
  convention), positive x points right.

Two control inputs, both passed via `params` rather than as separate
function arguments (per the assignment's instructions):
  - params["ankle_torque"]    : u1 = tau, continuous-time control, set
                                 fresh every integration timestep.
  - params["angle_of_attack"] : u2 = alpha, discrete-time control, held
                                 fixed for one whole stance phase (only
                                 changed between steps, right after
                                 event_dynamics fires).

With ankle_torque=0 this reduces exactly to an undriven rimless-wheel
stance leg -- same physics as Assignment 1, just phrased with these
...
def generate_params():
    params = {
        ...
        "incline": 0.06,  # ground slope (rad), positive descends to the right
        "angle_of_attack": np.pi / 8,  # half-angle between legs (rad), u2
        "ankle_torque": 0.0,  # ankle torque (N m), u1, default off
    }
    return params
...
def dynamics(t, state, params):
    ...
    # Undriven inverted-pendulum swing (same as the rimless wheel) plus
    # the ankle torque, summed directly into the numerator -- same
    # pattern as pendulum.py's damping term.
...
def event_guard(previous_state, next_state, params):
    """
    Detects whether a touchdown event happened between two consecutive
    integrator samples, by checking whether theta crossed the current
    touchdown angle theta_TD = incline +/- angle_of_attack over the step.

    Uses BOTH previous_state and next_state (rather than testing
    next_state alone) so a single step that jumps straight past the
    guard is still caught -- important here since assignment_2.py takes
    a small but nonzero fixed Euler step each iteration.

    theta_TD depends on the CURRENT stance phase's angle_of_attack
    control (u2), held fixed for this phase -- read directly out of
    params, exactly like ankle_torque is read for u1.

    Returns True if either the forward guard (swing leg strikes ground
    ahead, theta increasing through incline+angle_of_attack) or the
    backward guard (theta decreasing through incline-angle_of_attack,
    i.e. falling/stepping backward) was crossed this step; False
    otherwise.
    """
...
def event_dynamics(state, params):
    """
    Instantaneous plastic collision reset at touchdown: the leg that
    just struck the ground becomes the new stance leg (a coordinate
    shift by the current angle_of_attack), and the post-impact velocity
    conserves angular momentum about the new contact point:

        theta+    = theta-  -/+ 2*angle_of_attack
        thetadot+ = thetadot- * cos(2*angle_of_attack)

    Direction (-/+) is inferred from the sign of the incoming angular
    velocity, since event_guard doesn't pass along which guard fired.
    ankle_torque plays no role here: it's a finite force, so it
    contributes zero impulse over an instantaneous collision.

    Uses the SAME angle_of_attack that was active during the stance
    phase that just ended -- update params["angle_of_attack"] to your
    NEXT step's chosen control only after calling this, not before.
    """
...
def calculate_energy(state, params):
    ...
    Same form as pendulum.py's calculate_energy -- energy relative to
    the stance pivot only; the ground incline doesn't enter here since
    it only affects touchdown geometry, not the pendulum's height above
    its own pivot.
    """
    gravity = params["gravity"]
    length = params["length"]
